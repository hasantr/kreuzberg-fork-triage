...
pub trait OcrBackend: Plugin {
    ...
    /// Process a decoded RGB image and extract text via OCR.
    ///
    /// Preferred entry point for extractors that already have raw pixel data
    /// (e.g. PDF page renderer), avoiding a PNG encode → decode round-trip.
    /// For a typical 2480×3508 page this saves ~50-200ms of encode/decode work
    /// per call.
    ///
    /// # Arguments
    ///
    /// * `rgb` - Tightly-packed RGB8 pixel buffer; length MUST equal
    ///   `width * height * 3`
    ...
    ///
    /// Encodes the pixel buffer to PNG and delegates to [`process_image`],
    /// preserving backward compatibility — existing backends work unchanged.
    ///
    /// Custom backends that can consume raw pixels directly (e.g. to skip the
    /// PNG decode cost) should override this method for the fast path.
    ///
    /// [`process_image`]: OcrBackend::process_image
    async fn process_image_raw(
        ...
        let bytes = encode_rgb_to_png(rgb, width, height)?;
        self.process_image(&bytes, config).await
    }
...
/// Encode a tightly-packed RGB8 pixel buffer to a PNG byte vector.
///
/// Used as the default fallback in [`OcrBackend::process_image_raw`] to keep
/// existing backends backward-compatible when extractors feed them raw pixels.
/// Uses the lightweight `png` crate so this path is available in every
/// feature configuration, not just ones that pull in the `image` crate.
fn encode_rgb_to_png(rgb: &[u8], width: u32, height: u32) -> Result<Vec<u8>> {
    let expected = (width as usize)
        .checked_mul(height as usize)
        .and_then(|n| n.checked_mul(3))
        .ok_or_else(|| crate::KreuzbergError::Validation {
            message: "RGB buffer dimensions overflow".to_string(),
            source: None,
        })?;
    if rgb.len() != expected {
    ...
    let mut buf = Vec::with_capacity(expected / 2);
    {
        ...
        let mut writer = encoder
            .write_header()
            .map_err(|e| crate::KreuzbergError::Other(format!("PNG header: {e}")))?;
        writer
            .write_image_data(rgb)
            .map_err(|e| crate::KreuzbergError::Other(format!("PNG data: {e}")))?;
    }
    Ok(buf)
}
