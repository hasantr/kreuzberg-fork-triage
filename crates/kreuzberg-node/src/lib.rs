...
// alef:hash:a155a690c40d35d3a12b29f8ca626f1a230a2a3821c51383f098acfae7af1fcd
...
impl kreuzberg::plugins::PostProcessor for JsPostProcessorBridge {
    ...
    fn estimated_duration_ms(&self, _result: &kreuzberg::ExtractionResult) -> u64 {
        ...
        let result = func.call((match self.env().create_string(&format!("{:?}", _result)) {
            Ok(s) => s.to_unknown(),
            Err(_) => unsafe {
                let r = napi::bindgen_prelude::ToNapiValue::to_napi_value(
                    std::ptr::null_mut(),
...
impl kreuzberg::plugins::EmbeddingBackend for JsEmbeddingBackendBridge {
    ...
    async fn embed(&self, texts: Vec<String>) -> std::result::Result<Vec<Vec<f32>>, kreuzberg::KreuzbergError> {
        ...
        let result = func.call((match self.env().create_string(&format!("{:?}", texts)) {
            Ok(s) => s.to_unknown(),
            Err(_) => unsafe {
                let r = napi::bindgen_prelude::ToNapiValue::to_napi_value(
                    std::ptr::null_mut(),
...
impl From<JsExtractionConfig> for kreuzberg::ExtractionConfig {
    fn from(val: JsExtractionConfig) -> Self {
        Self {
            ...
            result_format: val.result_format.map(Into::into).unwrap_or_default(),
...
impl From<JsImageExtractionConfig> for kreuzberg::ImageExtractionConfig {
    fn from(val: JsImageExtractionConfig) -> Self {
        Self {
            ...
            classify: val.classify.unwrap_or_default(),
        }
    }
}
...
impl From<JsTokenReductionOptions> for kreuzberg::TokenReductionOptions {
    fn from(val: JsTokenReductionOptions) -> Self {
        Self {
            ...
            preserve_important_words: val.preserve_important_words.unwrap_or_default(),
        }
    }
}
...
impl From<JsLanguageDetectionConfig> for kreuzberg::LanguageDetectionConfig {
    fn from(val: JsLanguageDetectionConfig) -> Self {
        Self {
            ...
            detect_multiple: val.detect_multiple.unwrap_or_default(),
        }
    }
}
...
impl From<JsExtractionResult> for kreuzberg::ExtractionResult {
    fn from(val: JsExtractionResult) -> Self {
        Self {
            ...
            mime_type: (val.mime_type.unwrap_or_default()).into(),
    ...
    fn from(val: JsMetadata) -> Self {
        Self {
            ...
            additional: val
                ...
                .unwrap_or_default()
                .into_iter()
                .map(|(k, v)| (k.into(), serde_json::from_str(&v).unwrap_or(serde_json::json!(v))))
                .collect(),
        }
    }
}
...
impl From<JsArchiveMetadata> for kreuzberg::ArchiveMetadata {
    fn from(val: JsArchiveMetadata) -> Self {
        Self {
            format: (val.format.unwrap_or_default()).into(),
...
impl From<JsLayoutRegion> for kreuzberg::LayoutRegion {
    fn from(val: JsLayoutRegion) -> Self {
        Self {
            class_name: val.class_name.unwrap_or_default(),
