...
// alef:hash:fb08669a16a491e300b0196d90a809ec6f0f1091ffbc3ef5294fa2ddb80b83a5
...
use kreuzberg::{
    batch_extract_bytes, batch_extract_bytes_sync, extract_bytes, extract_bytes_sync, extract_file, extract_file_sync,
};
...
fn test_config_document_structure() {
    ...
    assert!(
        format!("{:?}", result.document.as_ref().unwrap().node_types).contains(r#"paragraph"#),
        "expected to contain: {}",
        r#"paragraph"#
    );
}
...
fn test_config_images() {
    ...
    let result = extract_file_sync(path, mime_type.as_deref(), &config).expect("should succeed");
    assert_eq!(result.mime_type.trim(), r#"application/pdf"#, "equals assertion failed");
    assert!(result.images.as_ref().is_some_and(|v| !v.is_empty()), "expected >= 1");
}
...
fn test_config_language_detection() {
    ...
    let config = serde_json::from_value(config_json).unwrap();
    let result = extract_file_sync(path, mime_type.as_deref(), &config).expect("should succeed");
    let detected_languages = result.detected_languages.as_deref().unwrap_or(&[]);
    assert_eq!(result.mime_type.trim(), r#"application/pdf"#, "equals assertion failed");
    assert!(
        result.content.len() >= 10,
        "expected length >= 10, got {}",
    ...
    assert!(
        format!("{:?}", detected_languages).contains(r#"eng"#),
        "expected to contain: {}",
        r#"eng"#
    );
}
...
fn test_result_format_unified() {
    ...
    assert!(
        result.elements.as_ref().is_none_or(|v| v.is_empty()),
        "expected elements to be empty or absent"
    );
}
