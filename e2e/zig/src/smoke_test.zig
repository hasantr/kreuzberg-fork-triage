...
// alef:hash:53719b1240a44ae04f0016886243b3847e6f0d84cd7f6ce244ebb66d7c647dbd
...
test "ocr_image_png" {
    ...
    const data_bytes = try std.Io.Dir.cwd().readFileAlloc(std.testing.io, "images/test_hello_world.png", std.heap.c_allocator, .unlimited);
    defer std.heap.c_allocator.free(data_bytes);
    const _result_json = try kreuzberg.extract_bytes_sync(data_bytes, "image/png", "{}");
    defer std.heap.c_allocator.free(_result_json);
    var _parsed = try std.json.parseFromSlice(std.json.Value, allocator, _result_json, .{});
    defer _parsed.deinit();
    const result = &_parsed.value;
    try testing.expectEqualStrings("image/png", result.object.get("mime_type").?.string);
...
test "smoke_html_basic" {
    ...
    defer _parsed.deinit();
    const result = &_parsed.value;
    try testing.expectEqualStrings("text/html", result.object.get("mime_type").?.string);
...
test "smoke_json_basic" {
    ...
    defer _parsed.deinit();
    const result = &_parsed.value;
    try testing.expectEqualStrings("application/json", result.object.get("mime_type").?.string);
...
test "smoke_txt_basic" {
    ...
    defer _parsed.deinit();
    const result = &_parsed.value;
    try testing.expectEqualStrings("text/plain", result.object.get("mime_type").?.string);
    try testing.expect(result.object.get("content").?.string.len >= 5);
}
...
test "smoke_xlsx_basic" {
    ...
    defer _parsed.deinit();
    const result = &_parsed.value;
    try testing.expectEqualStrings("application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", result.object.get("mime_type").?.string);
