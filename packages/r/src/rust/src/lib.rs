...
// alef:hash:e9d24372c4e99f0e708e3550b3a4d00cff3f0c231037072a85d0bbe81017fbc2
...
pub fn new_tablegrid(
    rows: Option<i32>,
    cols: Option<i32>,
    cells: Option<Vec<GridCell>>
) -> TableGrid {
    let mut __out = <TableGrid>::default();
    if let Some(v) = rows { __out.rows = v; }
    if let Some(v) = cols { __out.cols = v; }
    if let Some(v) = cells { __out.cells = v; }
    __out
}
...
pub fn extract_file_sync(path: String, mime_type: Option<String>, config: Option<ExtractionConfig>) -> Result<ExtractionResult> {
    let config_core: kreuzberg::ExtractionConfig = config.expect("'config' is required").into();
    kreuzberg::extract_file_sync(std::path::PathBuf::from(path), mime_type.as_deref(), &config_core).map(Into::into).map_err(|e| e.to_string())
}
...
pub fn extract_bytes_sync(content: Vec<u8>, mime_type: String, config: ExtractionConfig) -> Result<ExtractionResult> {
    let config_core: kreuzberg::ExtractionConfig = config.into();
    kreuzberg::extract_bytes_sync(&content, &mime_type, &config_core).map(Into::into).map_err(|e| e.to_string())
}
...
pub fn batch_extract_files_sync(items: Vec<BatchFileItem>, config: ExtractionConfig) -> Result<Vec<ExtractionResult>> {
    ...
    let config_core: kreuzberg::ExtractionConfig = config.into();
    kreuzberg::batch_extract_files_sync(items_core, &config_core).map(|val| val.into_iter().map(Into::into).collect()).map_err(|e| e.to_string())
}
...
#[allow(clippy::missing_errors_doc)]
#[extendr]
pub async fn batch_extract_files(items: Vec<BatchFileItem>, config: ExtractionConfig) -> Result<Vec<ExtractionResult>> {
    let items_core: Vec<_> = items.into_iter().map(Into::into).collect();
    let config_core: kreuzberg::ExtractionConfig = config.into();
    let rt = tokio::runtime::Runtime::new()?;
...
#[allow(clippy::missing_errors_doc)]
#[extendr]
pub async fn batch_extract_bytes(items: Vec<BatchBytesItem>, config: ExtractionConfig) -> Result<Vec<ExtractionResult>> {
    let items_core: Vec<_> = items.into_iter().map(Into::into).collect();
    let config_core: kreuzberg::ExtractionConfig = config.into();
    let rt = tokio::runtime::Runtime::new()?;
...
pub fn list_document_extractors() -> Result<Vec<String>> {
    kreuzberg::list_document_extractors().map_err(|e| e.to_string())
}
