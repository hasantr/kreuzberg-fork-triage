...
/**
 * Allocates Panama FFM upcall stubs for an IRenderer implementation, assembles the C vtable in native memory, and
 * provides static registerRenderer/unregisterRenderer helpers.
 */
public final class RendererBridge implements AutoCloseable {
    ...
    private int handleRender(MemorySegment userData, MemorySegment doc_in, MemorySegment outResult,
    ...
    public static void registerRenderer(final IRenderer impl) throws Exception {
        ...
        try {
            try (var nameArena = Arena.ofConfined()) {
                ...
                int rc = (int) NativeLib.KREUZBERG_REGISTER_RENDERER.invoke(nameCs, bridge.vtableSegment(),
