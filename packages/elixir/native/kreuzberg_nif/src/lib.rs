...
// alef:hash:848d41e044ad91bf1a1c057171e6914ac714b3a201d4d0437376cef2c6a81743
...
impl std::panic::RefUnwindSafe for TesseractWasmBackend {}

impl rustler::Resource for TesseractWasmBackend {}
...
#[rustler::nif]
pub fn tesseractwasmbackend_name(resource: rustler::ResourceArc<TesseractWasmBackend>) -> String {
    resource.inner.name().into()
}

#[rustler::nif]
pub fn tesseractwasmbackend_version(resource: rustler::ResourceArc<TesseractWasmBackend>) -> String {
    resource.inner.version()
}

#[rustler::nif]
pub fn tesseractwasmbackend_initialize(resource: rustler::ResourceArc<TesseractWasmBackend>) -> Result<(), String> {
    let result = resource.inner.initialize().map_err(|e| e.to_string())?;
    Ok(result)
}

#[rustler::nif]
pub fn tesseractwasmbackend_shutdown(resource: rustler::ResourceArc<TesseractWasmBackend>) -> Result<(), String> {
    let result = resource.inner.shutdown().map_err(|e| e.to_string())?;
    Ok(result)
}

#[rustler::nif(schedule = "DirtyCpu")]
pub fn tesseractwasmbackend_process_image_async(
    resource: rustler::ResourceArc<TesseractWasmBackend>,
    image_bytes: Vec<u8>,
    _config: OcrConfig,
) -> Result<ExtractionResult, String> {
    Err(String::from(
        "Not implemented: tesseractwasmbackend_process_image_async",
    ))
}

#[rustler::nif(schedule = "DirtyCpu")]
pub fn tesseractwasmbackend_process_image_file_async(
    resource: rustler::ResourceArc<TesseractWasmBackend>,
    path: String,
    config: OcrConfig,
) -> Result<ExtractionResult, String> {
    Err(String::from(
        "Not implemented: tesseractwasmbackend_process_image_file_async",
    ))
}

#[rustler::nif]
pub fn tesseractwasmbackend_supports_language(
    resource: rustler::ResourceArc<TesseractWasmBackend>,
    _lang: String,
) -> bool {
    resource.inner.supports_language(&_lang)
}

#[rustler::nif]
pub fn tesseractwasmbackend_backend_type(resource: rustler::ResourceArc<TesseractWasmBackend>) -> OcrBackendType {
    resource.inner.backend_type().into()
}
...
fn on_load(env: rustler::Env, _info: rustler::Term) -> bool {
    ...
    env.register::<TesseractWasmBackend>()
        .expect("Failed to register resource type TesseractWasmBackend");
    true
}
