...
// alef:hash:4327e8a53c6ac61fb37e4170e9ffaa7ad44d8c3b35a83c696123b486f648b0db
...
extern int32_t goDocumentExtractorAsSyncExtractor(void* user_data, char** out_result, char** out_error);
...
// unregister_ocr_backend unregisters a OcrBackend implementation by name.
func unregister_ocr_backend(name string) error {
	cName := C.CString(name)
	defer C.free(unsafe.Pointer(cName))
	var cErr *C.char
	rc := C.kreuzberg_unregister_ocr_backend(cName, &cErr)
...
// unregister_post_processor unregisters a PostProcessor implementation by name.
func unregister_post_processor(name string) error {
	cName := C.CString(name)
	defer C.free(unsafe.Pointer(cName))
	var cErr *C.char
	rc := C.kreuzberg_unregister_post_processor(cName, &cErr)
...
// unregister_validator unregisters a Validator implementation by name.
func unregister_validator(name string) error {
	cName := C.CString(name)
	defer C.free(unsafe.Pointer(cName))
	var cErr *C.char
	rc := C.kreuzberg_unregister_validator(cName, &cErr)
...
// unregister_embedding_backend unregisters a EmbeddingBackend implementation by name.
func unregister_embedding_backend(name string) error {
	cName := C.CString(name)
	defer C.free(unsafe.Pointer(cName))
	var cErr *C.char
	rc := C.kreuzberg_unregister_embedding_backend(cName, &cErr)
...
type DocumentExtractor interface {
	...
	Shutdown() error
	// extract_bytes
	ExtractBytes(content []byte, mime_type string, config map[string]interface{}) (map[string]interface{}, error)

	// extract_file
	ExtractFile(path interface{}, mime_type string, config map[string]interface{}) (map[string]interface{}, error)
	...
	// can_handle
	CanHandle(_path interface{}, _mime_type string) bool

	// as_sync_extractor
	AsSyncExtractor() *map[string]interface{}
}
...
func goDocumentExtractorAsSyncExtractor(
	userData unsafe.Pointer,
	outResult **C.char,
	outError **C.char,
) C.int32_t {
...
func RegisterDocumentExtractor(impl DocumentExtractor) error {
	...
	vtable := C.KREUZBERGKreuzbergDocumentExtractorVTable{
		...
		as_sync_extractor:    (*[0]byte)(unsafe.Pointer(C.goDocumentExtractorAsSyncExtractor)),
...
// unregister_document_extractor unregisters a DocumentExtractor implementation by name.
func unregister_document_extractor(name string) error {
	cName := C.CString(name)
	defer C.free(unsafe.Pointer(cName))
	var cErr *C.char
	rc := C.kreuzberg_unregister_document_extractor(cName, &cErr)
...
type Renderer interface {
	...
	Shutdown() error
	// render
	Render(doc map[string]interface{}) (string, error)
}
...
func goRendererRender(
	...
	if doc != nil {
		var rawData interface{}
		json.Unmarshal([]byte(C.GoString(doc)), &rawData)
...
// unregister_renderer unregisters a Renderer implementation by name.
func unregister_renderer(name string) error {
	cName := C.CString(name)
	defer C.free(unsafe.Pointer(cName))
	var cErr *C.char
	rc := C.kreuzberg_unregister_renderer(cName, &cErr)
